"""
CareerPilot — Dashboard Frontend HTML Template
"""
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .cover-box { background: var(--surface2); border: 1px solid var(--border); border-radius: 12px; padding: 1rem; font-family: monospace; font-size: 0.82rem; white-space: pre-wrap; color: #dfe4ea; margin: 1rem 0; line-height: 1.5; }
...
<header>
  ...
  <div class="status-pill">
    ...
    <span>4 Agents Active (Commander, Tracker, Predictor, Applier)</span>
  </div>
</header>
...
<div class="container">
  ...
  <div class="grid-profile">
    ...
    <div class="card">
      ...
      <form class="profile-form" onsubmit="event.preventDefault(); saveProfile();">
        ...
        <input type="text" id="prof-skills" class="input-field" value="Python, FastAPI, React, Machine Learning, SQL, Docker, Git"/>
    ...
    <div class="card">
      ...
      <div class="presets-bar">
        <div class="preset-chip" onclick="setGoal('Find 20 Python & Machine Learning Internships in India')">🎯 Python Internships</div>
        <div class="preset-chip" onclick="setGoal('Find 15 AI Engineer Remote Jobs with high fit score')">🌐 Remote AI Jobs</div>
        <div class="preset-chip" onclick="setGoal('Find 20 Data Scientist Jobs in Bangalore')">📍 Bangalore Data Roles</div>
  ...
  <!-- Applications Table Container (Hidden by default) -->
  <div id="apps-container" class="card" style="display:none">
    <div class="card-title">📑 Applied Opportunities & Status</div>
    <div id="apps-table-wrap">Loading applications...</div>
  </div>

</div>

<!-- Modal for Application / Cover Letter Preview -->
...
async function loadJobs() {
  const typeParam = currentTab === 'applications' ? 'all' : currentTab;
  const res = await fetch(`/api/jobs?job_type=${typeParam}&per_page=60`);
  const data = await res.json();
  allJobs = data.jobs || [];
  document.getElementById('cnt-all').innerText = data.total || allJobs.length;
  renderJobs(allJobs);
}
...
function renderJobs(jobs) {
  ...
  if (!jobs.length) {
    container.innerHTML = `<div style="grid-column: 1/-1; text-align:center; padding:3rem; color:var(--text-dim)">No opportunities found in this category.</div>`;
    return;
  }

  const appJobIds = new Set(allApps.map(a => a.job_id));

  container.innerHTML = jobs.map(j => {
    const isApplied = appJobIds.has(j.id);
    const fit = j.fit_score ? `${j.fit_score}% Fit` : '85% Fit';
    const isIntern = j.is_internship || j.title.toLowerCase().includes('intern');
        ...
        <div>
          <div class="job-header">
            <div>
              ...
              <div class="company-name">${j.company} • ${j.location || 'Remote'}</div>
            </div>
          </div>
          
          <div class="badges">
            <span class="badge ${isIntern ? 'badge-intern' : 'badge-job'}">${isIntern ? '🎓 INTERNSHIP' : '💼 FULL-TIME'}</span>
            <span class="badge badge-fit">✨ ${fit}</span>
            ${j.is_remote ? `<span class="badge badge-remote">🌐 Remote</span>` : ''}
          </div>

          <div class="job-skills">
            ${(j.matched_skills || ['Python', 'AI']).map(s => `<span class="job-skill-chip">${s}</span>`).join('')}
          </div>
        </div>
...
async function dispatchGoal() {
  ...
  alert('Goal submitted to Commander Agent!');
}
